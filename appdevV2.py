...
def generate_executive_summary(client, model_name, client_name):
    """Generate ONLY the Executive Summary for an AppDev SOW"""

    prompt = f"""
You are a Senior Custom Application Development Consultant from Crave InfoTech.

Generate ONLY the Executive Summary section for a Custom Application Development (e.g., Fiori, Mobility, RPA) SOW.
...
- Provide a high-level overview of the client’s need for custom applications (e.g., digitalizing processes, mobile access, automating tasks, improving user experience).
- Mention how the proposed solution (e.g., Fiori apps, mobile solutions, custom portal) will bridge functional gaps, enhance operational efficiency, and drive user adoption.
- Highlight the strategic value: leveraging modern technology (e.g., SAP BTP, modern frameworks) for agility, scalability, and innovation.
- Emphasize the alignment with the client's digital transformation roadmap.
...
INSTRUCTIONS:
- Describe Crave InfoTech's expertise in Custom App Development, Fiori, Mobility, and SAP BTP in 2-3 lines
- Highlight expertise in user-centric design and agile delivery
...
Generate the "Our Understanding & Solution" section for {client_name}'s Custom App Development SOW.
DO NOT use bold (**text**) or markdown. Headings must be plain text.

CRITICAL:
- Keep it high-level, focusing on business value and user experience.
- Total length should be: 5–7 paragraphs + a short bullet list.
- Each paragraph must be 4–5 lines.
- Do NOT include low-level technical details (no specific code snippets, database schemas, or API names).

MANDATORY STRUCTURE:

3.1 Our Understanding
Write 2–3 paragraphs:
- Understanding of the client's current process gaps, poor user experience (UX), or mobility needs.
- The business drivers for the custom application (e.g., efficiency gains, better data capture, compliance).
- Strategic need for modernization: empowering users with simple, intuitive applications.

3.2 Our Proposed Solution
Write 2–3 paragraphs:
- A high-level Agile/Scrum application development approach (e.g., iterative development, frequent feedback).
- Why the chosen technology stack (e.g., Fiori/BTP, mobile framework) is suitable for solving the business problem.
- Business benefits: improved productivity, reduced training costs, real-time data access, and high user satisfaction.
- Keep content business-oriented, focusing on "what" and "why," not "how."

3.3 Challenges They Are Facing
Provide 5–6 bullets:
- High-level project, organizational, and technical challenges.
- Focus on change management, gathering clear and stable requirements, integration complexity (especially with legacy systems), and ensuring security standards.
...
def generate_project_scope(client, model_name, client_name, total_interfaces=None):
    """
    Generate Project Scope with fixed SOW structure (AppDev Version)
    """

    prompt = f"""
You are writing the "Project Scope" section for {client_name}'s Custom Application Development project.
...
- The scope of the custom application(s) to be developed (e.g., specific Fiori apps, a mobile module).
- The key business processes that will be digitized or automated.
- The chosen development methodology (e.g., iterative, design thinking focus).
- The strategic value of delivering a production-ready, highly usable application.

4.2 Deliverables
- List all project deliverables.
- Final Application Codebase and deployed binaries.
- Functional and Technical Design Specifications (including UX/UI mockups).
- Test plans, test cases, and unit test results.
- End-user training materials and developer knowledge transfer.

4.3 Acceptance Criteria
- Clear criteria for successful application acceptance.
- Successful completion of all user stories/functional requirements.
- Positive User Acceptance Testing (UAT) sign-off on functionality and performance.
- Application meets defined performance and security standards.
- Successful deployment to the target environment.

4.4 Out of Scope
- Explicit list of exclusions for this development project.
- Major changes to the underlying SAP/Non-SAP backend systems.
- Development of unrelated applications or features not covered by agreed user stories.
- End-user device provisioning or infrastructure setup outside of the core application hosting.
...
Generate ONLY a short introductory narrative (2–3 lines) for the "Project Delivery Approach" section for {client_name}'s AppDev project.

CONTEXT:
The detailed project phase table (e.g., Discover, Design/Prototype, Develop/Test, Deploy) will already be included in the SOW template. Your output should ONLY provide a high-level introduction explaining the overall development methodology (e.g., an Agile/Scrum or iterative approach).
...
- Explains Crave InfoTech's iterative and user-centric development methodology.
- Mentions the focus on continuous feedback, sprint planning, and rapid prototyping.
- Summarizes how UX/UI designers, developers, and client product owners collaborate to deliver high-quality code.
...
def generate_timelines(client, model_name, client_name, total_interfaces=None):
    ...
    prompt = f"""
Write the full 'Project Timelines & Resources' section for {client_name}'s Custom Application Development project.
Produce 3 paragraphs of 4–5 lines each. No bullets, no headings, no dates, no numbers.

Guidelines:
- Paragraph 1: Describe the overall project timeline in terms of sprints, releases, and key milestones (e.g., design sign-off, UAT completion, production cutover) using an iterative model.
- Paragraph 2: Explain how Crave and client teams coordinate resources, ensuring involvement of Product Owners, Subject Matter Experts (SMEs), and dedicated testing resources across sprints.
- Paragraph 3: Summarize post-go-live support, knowledge transfer to maintenance teams, and how this iterative approach ensures business value is realized quickly and continuously.

Do NOT mention technical configurations, specific programming languages, or low-level interface details.
...
def generate_payment_terms(client, model_name, client_name):
    ...
    prompt = f"""
Write the 'Payment Terms' section for {client_name}'s Custom Application Development SOW.

CONTENT:
- Provide a milestone-based payment structure.
- Use 4–6 milestones corresponding to project phases (e.g., Design Sign-off, Alpha Release, Beta Release, Production Go-Live, Post Go-Live).
...
def generate_key_assumptions(client, model_name, client_name):
    ...
    prompt = f"""
You are writing the "Key Assumptions" section for {client_name}'s Custom Application Development SOW.

GENERATE:
- 3 to 5 subsections with simple headings (e.g., Client Responsibilities, Requirements Stability, Infrastructure & Access, Project Governance).
- Each subsection should contain 2–4 short bullet points.
- Bullets must be high-level and project-oriented.
- Content must be based on standard AppDev project assumptions (e.g., client provides timely feedback, requirements are stable after design sign-off, necessary API/security access is provided).
...
- Do NOT mention specific technical details.
...
def main():
    ...
    st.session_state.setdefault("client_name_appdev", "") # Unique key
    st.session_state.setdefault("uploaded_file_appdev", None) # Unique key
    ...
    with st.expander("⚙️ Input Configuration", expanded=True):
        ...
        client_name = st.text_input("Enter Client Name (required)", st.session_state["client_name_appdev"])
        st.session_state["client_name_appdev"] = client_name # Update session state

        # File upload (UPDATED to use session state key and track changes)
        uploaded_file = st.file_uploader(
            "Upload RFP Document",
            type=["pdf", "docx", "xlsx", "pptx"],
            key="rfp_uploader_appdev", # Unique key
            help="Upload PDF, Word, Excel or PowerPoint reference document.",
        )
        # Handle file upload change (NEW LOGIC)
        if uploaded_file != st.session_state["uploaded_file_appdev"]:
            st.session_state["uploaded_file_appdev"] = uploaded_file
        ...
        if "checkbox_states_appdev" not in st.session_state: # Unique key
            st.session_state["checkbox_states_appdev"] = {section: False for section in SECTION_LIST}
        ...
        for i, section in enumerate(SECTION_LIST):
            ...
            with col:
                # Get current state
                current_state = st.session_state["checkbox_states_appdev"][section]
                # Use a unique key for checkboxes in AppDev
                new_state = st.checkbox(section, value=current_state, key=f"chk_appdev_{section}_v2")
                
                # Update state if changed
                st.session_state["checkbox_states_appdev"][section] = new_state
        
        # Build selected sections list in the order they appear in SECTION_LIST
        selected_sections = [s for s in SECTION_LIST if st.session_state["checkbox_states_appdev"].get(s)]
        ...
        if st.button("⚡ Generate Content", key="gen_appdev"): # Unique key
            ...
            # Use uploaded_file_appdev from session state which is the source of truth after the rerun logic
            uploaded_file_sot = st.session_state["uploaded_file_appdev"]
    ...
    if "edited_sections" in st.session_state:
        ...
        template_path = "Template/AppDev_Template.docx"
        
        # If V2 template doesn't exist yet, fallback to original
        if not os.path.exists(template_path):
            st.warning("⚠ Template not found, please create AppDev_Template.docx")
            template_path = "Template/AppDev_Template.docx"
